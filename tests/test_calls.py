...
def test_create_call_sends_auth_and_idempotency_headers() -> None:
    route = respx.post("https://api.example.com/v1/calls").mock(return_value=httpx.Response(200, json=COMPLETED_CALL))
    client = CalleClient(api_key="key_test", base_url="https://api.example.com")
...
def test_create_call_maps_api_errors() -> None:
    respx.post("https://api.example.com/v1/calls").mock(
...
def test_wait_for_result_returns_completed_call() -> None:
    ...
    respx.get("https://api.example.com/v1/calls/call_123").mock(
...
def test_wait_for_result_returns_failed_call() -> None:
    ...
    respx.get("https://api.example.com/v1/calls/call_123").mock(return_value=httpx.Response(200, json=failed))
    client = CalleClient(api_key="key_test", base_url="https://api.example.com")
...
def test_wait_for_result_raises_timeout() -> None:
    queued = {**COMPLETED_CALL, "status": "queued", "structured_result": None, "completed_at": None}
    respx.get("https://api.example.com/v1/calls/call_123").mock(return_value=httpx.Response(200, json=queued))
    client = CalleClient(api_key="key_test", base_url="https://api.example.com")
